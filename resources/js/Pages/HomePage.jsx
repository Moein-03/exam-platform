...
const HomePage = ({ auth }) => {
  ...
  const isTeacher = isAuthenticated && auth.user.role === 'teacher';
  const isStudent = isAuthenticated && auth.user.role === 'student';
  ...
  const cards = [
    ...
    {
      title: 'سوالات',
      description: isTeacher ? 'بانک سوالات خود را مدیریت کنید، سوال جدید بسازید یا ویرایش کنید' : 'سوالات نمونه را مشاهده کنید (فقط مطالعه)',
  ...
  if (isStudent) {
    cards.push({
      ...
      link: '/exams', // می‌توانید به صفحه خاص نتایج هدایت کنید
    ...
    <Box sx={{ bgcolor: '#f5f7fa', minHeight: '100vh' }}>
      ...
      >
        <Container maxWidth="lg">
          ...
          {isAuthenticated && (
            <Box sx={{ mt: 4 }}>
              <Typography variant="body1" sx={{ bgcolor: 'rgba(255,255,255,0.2)', display: 'inline-block', p: 1, borderRadius: 2 }}>
